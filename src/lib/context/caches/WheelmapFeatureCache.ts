...
import { convertResponseToWheelmapFeature } from '../types/Feature';
import { WheelmapFeature, WheelmapFeatureCollection } from '../types/Feature';
...
> {
  ...
  static getFeatureFromResponse(response): Promise<WheelmapFeature> {
    ...
    return response
      .json()
      .then(responseJson =>
        convertResponseToWheelmapFeature(responseJson.node),
      );
  }
}

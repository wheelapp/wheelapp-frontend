...
import { t } from 'ttag';
import { normalizeCoordinates } from '../geo/normalizeCoordinates';
import { SearchResultFeature } from '../global-context/api/searchPlaces';
import { LocalizedString, translatedStringFromObject } from '../i18n';
import {
  Category,
  categoryNameFor,
  getCategoryIdFromProperties,
} from './Categories';
import {
  EquipmentInfo,
...
export interface ClassicCategoryOrNodeType {
  id: number | null;
...
export interface WheelmapProperties {
  id: number;
  category: ClassicCategoryOrNodeType | null;
  node_type: ClassicCategoryOrNodeType | null;
  city: string | null;
...
export type AccessibilityCloudFeatureCollection = FeatureCollection<
  AccessibilityCloudFeature
>;
export type WheelmapFeatureCollection = FeatureCollection<WheelmapFeature>;
export type WheelmapLightweightFeatureCollection = FeatureCollection<
...
export function getFeatureId(
  feature: Feature | EquipmentInfo | any,
): string | null {
  ...
  const idProperties = [
    ...
    feature.properties &&
      typeof feature.properties._id === 'string' &&
...
export function hrefForFeature(
  ...
  if (!featureId) {
    throw new Error(
      'Could not create href because featureId seems to be not defined',
    );
  }
...
export function isWheelmapFeatureId(
  id: string | number | null | void,
): boolean {
  return typeof id !== 'undefined' && isNumeric(id);
}

export function isWheelmapFeature(
  feature: Feature,
): feature is WheelmapFeature {
  return isWheelmapFeatureId(feature['id']);
}

export function wheelmapFeatureFrom(
...
export function accessibilityCloudFeatureFrom(
  feature: Feature | null,
...
export function isWheelmapProperties(
  properties: WheelmapProperties | AccessibilityCloudProperties,
): properties is WheelmapProperties {
...
export function sourceIdsForFeature(feature: Feature | any): string[] {
  ...
  const idsToEquipmentInfos =
    typeof properties.equipmentInfos === 'object'
      ? properties.equipmentInfos
      : null;
  ...
  return uniq(
    [placeSourceId, ...equipmentInfoSourceIds, ...disruptionSourceIds].filter(
      Boolean,
    ),
  );
}

export function convertResponseToWheelmapFeature(
  node: WheelmapProperties,
): WheelmapFeature {
  return {
    type: 'Feature',
    properties: node,
...
export function hasAccessibleToilet(
  ...
  if (isToilet) {
    return allowToiletsOfAnyAccessibility || isAccessible === 'yes'
      ? 'yes'
      : 'no';
  }
...
function hasAccessibleToiletLegacyAcFormat(
  ...
  if (!(get(properties, 'accessibility.areas') instanceof Array))
    return 'unknown';
  ...
  const restroomInfos = flatten(
    properties.accessibility.areas.map(area => {
      ...
      return area.restrooms.map(
        restroom => restroom.isAccessibleWithWheelchair,
      );
    }),
  );
  ...
  const unknownCount = restroomInfos.filter(
    a => a === null || typeof a === 'undefined',
  ).length;
...
function hasAccessibleToiletAcFormat(
  ...
  const restroomInfos = restrooms.map(
    restroom => restroom.isAccessibleWithWheelchair,
  );
...
export function isWheelchairAccessible(
  ...
  if (
    ...
    properties.category === 'escalator'
  ) {
    const result =
      // @ts-ignore
      isEquipmentAccessible(pick(properties, ['lastUpdate', 'isWorking'])) ||
      'unknown';
    // debugger
    return result;
  }
